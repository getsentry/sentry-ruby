...
RSpec.describe Sentry::Event do
  ...
  describe "#initialize" do
    ...
    described_class::REQUIRED_OPTION_KEYS.each do |required_key|
      it "raises error if #{required_key} is not given" do
        ...
        end.to raise_error("you much provide configuration when initializing a Sentry::Event")
      end
    end
  end
  ...
  context 'parameter entries are nil' do
    ...
    it "skips nil values" do
      expect(hash[:extra]).to eq(Sentry.context.extra)
      expect(hash[:user]).to eq(Sentry.context.user)
      expect(hash[:tags]).to eq(Sentry.configuration.tags)
    end
  end

  context 'user context specified' do
    let(:hash) do
      Sentry.user_context('id' => 'hello')
  ...
  context 'tags context specified' do
    let(:hash) do
      Sentry.tags_context('key' => 'value')

      Sentry::Event.new(level: 'warning',
                       logger: 'foo',
                       tags: {
                         'foo' => 'bar'
                       },
  ...
  context 'extra context specified' do
    let(:hash) do
      Sentry.extra_context('key' => 'value')
  ...
  #   let(:hash) do
  #     Sentry.rack_context('REQUEST_METHOD' => 'POST',
  ...
  #   let(:hash) do
  #     Sentry.rack_context('REQUEST_METHOD' => 'GET',
  #                        'rack.url_scheme' => 'http',
  #                        'rack.input' => StringIO.new('a' * 4096 * 5))

  #     Sentry::Event.new(essential_options).to_hash
  #   end
  ...
  context 'configuration tags specified' do
    let(:hash) do
      ...
      Sentry::Event.new(
        ...
        breadcrumbs: Sentry.breadcrumbs
      ).to_hash
    end
  ...
  context 'merging user context' do
    before do
      Sentry.user_context('context_event_key' => 'context_value',
                         'context_key' => 'context_value')
    end

    let(:hash) do
      Sentry::Event.new(user: {
                         'context_event_key' => 'event_value',
                         'event_key' => 'event_value'
                       },
                       configuration: Sentry.configuration,
                       context: Sentry.context,
                       breadcrumbs: Sentry.breadcrumbs).to_hash
    end
  ...
  context 'merging extra context' do
    before do
      Sentry.extra_context('context_event_key' => 'context_value',
                          'context_key' => 'context_value')
    end
  ...
  describe '.to_json_compatible' do
    ...
    context "with sensitive data" do
      ...
      it "should sanitize password" do
        ...
        expect(json["extra"]["password"]).to eq(Sentry::Processor::SanitizeData::STRING_MASK)
      end
    end
  end
  ...
  describe '.capture_message' do
    ...
    let(:hash) { Sentry::Event.capture_message(message, essential_options).to_hash }

    context 'for a Message' do
      it 'returns an event' do
        expect(Sentry::Event.capture_message(message, essential_options)).to be_a(Sentry::Event)
      end

      it "doesn't change the option hash" do
        h_int = { abc: :abc }
        h = { k1: h_int, k2: h_int }
        Sentry.capture_message "Test extra", extra: { h1: h, h2: h_int }, **essential_options
      ...
      it 'accepts an options hash' do
        expect(Sentry::Event.capture_message(message, logger: 'logger', **essential_options).logger).to eq('logger')
      end

      it 'accepts a stacktrace' do
        backtrace = ["/path/to/some/file:22:in `function_name'",
                     "/some/other/path:1412:in `other_function'"]
        evt = Sentry::Event.capture_message(message, backtrace: backtrace, **essential_options)
        expect(evt[:stacktrace]).to be_a(Sentry::StacktraceInterface)
  ...
  describe '.capture_exception' do
    ...
    let(:hash) { Sentry::Event.capture_exception(exception, **essential_options).to_hash }

    context 'for an Exception' do
      it 'returns an event' do
        expect(Sentry::Event.capture_exception(exception, **essential_options)).to be_a(Sentry::Event)
      end
    ...
    context 'for a Sentry::Error' do
      ...
      it 'does not create an event' do
        expect(Sentry::Event.capture_exception(exception, **essential_options)).to be_nil
      end
    end
    ...
    context 'for an excluded exception type' do
      ...
      context "invalid exclusion type" do
        it 'returns Sentry::Event' do
          ...
          expect(Sentry::Event.capture_exception(Sentry::Test::BaseExc.new, essential_options)).to be_a(Sentry::Event)
        end
      end
      ...
      context "defined by string type" do
        it 'returns nil for a class match' do
          ...
          expect(Sentry::Event.capture_exception(Sentry::Test::BaseExc.new, essential_options)).to be_nil
        end
        ...
        it 'returns nil for a sub class match' do
          config.excluded_exceptions << 'Sentry::Test::BaseExc'
          expect(Sentry::Event.capture_exception(Sentry::Test::SubExc.new, essential_options)).to be_nil
        end

        it 'returns nil for a tagged class match' do
          config.excluded_exceptions << 'Sentry::Test::ExcTag'
          expect(
            Sentry::Event.capture_exception(
              Sentry::Test::SubExc.new.tap { |x| x.extend(Sentry::Test::ExcTag) },
              essential_options
            )
          ).to be_nil
        end
      ...
      context "defined by class type" do
        ...
        it 'returns nil for a tagged class match' do
          config.excluded_exceptions << Sentry::Test::ExcTag
          expect(Sentry::Event.capture_exception(Sentry::Test::SubExc.new.tap { |x| x.extend(Sentry::Test::ExcTag) }, essential_options)).to be_nil
        end
      end
    end
    ...
    it 'uses an annotation if one exists' do
      Sentry.annotate_exception(exception, logger: 'logger')
      expect(Sentry::Event.capture_exception(exception, **essential_options).logger).to eq('logger')
    end

    it 'accepts a checksum' do
      event = Sentry::Event.capture_exception(exception, checksum: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', **essential_options)
      expect(event.checksum).to eq('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
    end

    it 'accepts a release' do
      expect(Sentry::Event.capture_exception(exception, release: '1.0', **essential_options).release).to eq('1.0')
    end

    it 'accepts a fingerprint' do
      event = Sentry::Event.capture_exception(exception, fingerprint: ['{{ default }}', 'foo'], **essential_options)
      expect(event.fingerprint).to eq(['{{ default }}', 'foo'])
    end

    it 'accepts a logger' do
      expect(Sentry::Event.capture_exception(exception, logger: 'root', **essential_options).logger).to eq('root')
    end
  end
end
