...
RSpec.describe Sentry::Client do
  ...
  describe "#event_from_exception" do
    ...
    it 'accepts a checksum' do
      event = subject.event_from_exception(exception, checksum: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
      expect(event.checksum).to eq('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
    end

    it 'accepts a release' do
      expect(subject.event_from_exception(exception, release: '1.0').release).to eq('1.0')
    end

    it 'accepts a fingerprint' do
      event = subject.event_from_exception(exception, fingerprint: ['{{ default }}', 'foo'])
