...
RSpec.describe Sentry::Rails::LogSubscriber, type: :request do
  ...
  context "with no parameter filtering" do
    ...
    after do
      # Clear the logger between tests to prevent event accumulation
      Sentry.logger.clear if Sentry.logger.respond_to?(:clear)
      # Detach the test subscriber to prevent multiple subscriptions
      test_subscriber_class.detach_from(:test_component) if test_subscriber_class.respond_to?(:detach_from)
    end
  ...
  context "parameter filtering integration" do
    ...
    after do
      ...
      Sentry.logger.clear if Sentry.logger.respond_to?(:clear)
      # Detach the test subscriber to prevent multiple subscriptions
      test_subscriber_class.detach_from(:filtering_test) if test_subscriber_class.respond_to?(:detach_from)
    end
