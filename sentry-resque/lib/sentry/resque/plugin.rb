...
module Sentry
  module Resque
    class Plugin
      ...
      #       scope.set_tags("resque.queue" => job.queue, "resque.id" => job.id.to_s)
      ...
      def self.capture_exception(exception, job)
        Sentry::Resque.capture_exception(exception, hint: { background: false }) if report?(job)
      end

      def self.report?(job)
        return true unless Sentry.configuration.resque.report_after_job_retries
...
# Delayed::Worker.plugins << Sentry::Resque::Plugin
